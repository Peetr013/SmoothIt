...
const TYpage = () => {
  ...
  useEffect(() => {
    if (!orderId) {
      setError(
        "ID objednávky nebylo nalezeno v URL. Ujistěte se, že ID je předáno v URL (např. /TYpage/vaseID)."
      );
      setLoading(false);
      return;
    }
    ...
    const fetchOrderDetails = async () => {
      ...
      try {
        const response = await fetch(
          `${API_SINGLE_ORDER_DETAILS_URL_BASE}/${orderId}`
        );
        ...
        setError(
          `Nepodařilo se načíst detaily objednávky. Chyba: ${err.message}`
        );
    ...
    const interval = setInterval(() => {
      if (order && order.status === "done") {
        ...
        toast.success("Vaše smoothie je hotové! Můžete si pro něj přijít.", {
          duration: 8000,
        });
      } else {
        fetchOrderDetails();
