...
const OrderPage = () => {
  ...
  const handleFinishOrder = async () => {
    ...
    try {
      ...
      toast.success(`Objednávka odeslána! Celkem: ${totalPrice} Kč`, { id: 'orderSending' });
      setSelectedIngredientIds(new Set());
      ...
      <div className="flex flex-col min-h-[calc(100vh-64px)] bg-pink-100 text-black p-4">
        ...
        <div className="flex flex-wrap justify-center gap-4 mb-8">
          {ingredientsData.map((ingredient) => (
            ...
            >
              <img src={ingredient.image} alt={ingredient.name} className="w-24 h-24 mx-auto" />
              <p className="text-center font-semibold mt-2">{ingredient.name}</p>
              <p className="text-center text-sm text-gray-700">{ingredient.price} Kč</p>
            </div>
          ))}
        </div>
